...
export async function getRecordsForUser (userId) {
  ...
  return rows.map((r) => ({
    ...
    comentario_asesor: r.comentario_asesor,
    asesor: r.asesor
  }));
}
...
export async function createRecordForUser (userId, payload) {
  ...
  const { rows } = await db.query(
    `INSERT INTO ${SCHEMA}.comisiones (arancel, comentario_admin_contable, comentario_finanzas, estado_de_pago, descuento_aplicado, fecha_matricula, matricula, matriculados_programa, meta_asesor, metodo_de_pago, sede, valor_comision, version_programa, id_asesor, rut_estudiante, cod_programa)
