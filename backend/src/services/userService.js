...
export async function findUserByUsername(username) {
  const { rows } = await db.query(
    `SELECT u.id,
     ...
     WHERE u.username = $1
     GROUP BY u.id, u.username, u.password_hash, cu.nombre_completo, cu.bx24_id`,
    [username]
  );
...
export async function findUserById(id) {
  const { rows } = await db.query(
    `SELECT u.id,
     ...
     LEFT JOIN comision_ua.usuarios cu ON cu.id = u.id
     WHERE u.id = $1
     GROUP BY u.id, u.username, cu.nombre_completo, cu.bx24_id`,
    [id]
  );
  if (!rows[0]) return null;
  const user = rows[0];
...
export async function updateUserPassword (userId, newPassword) {
  ...
  await db.query('UPDATE usuarios SET password_hash = $1, actualizado_el = now() WHERE id = $2', [passwordHash, userId]);
