...
export async function findUserByUsername(username) {
  const { rows } = await db.query(
    `SELECT u.id, u.username, u.password_hash, u.bx24_id, array_agg(r.nombre) AS roles, array_agg(r.id) AS role_ids
     FROM usuarios u
  ...
  user.is_asesor = user.role_ids.includes(1);
  user.bx24_id = user.bx24_id || null;
  return user;
}

export async function findUserById(id) {
  const { rows } = await db.query(
    `SELECT u.id, u.username, u.bx24_id, array_agg(r.nombre) AS roles, array_agg(r.id) AS role_ids
     FROM usuarios u
    LEFT JOIN usuario_rol ur ON ur.id_usuario = u.id
    LEFT JOIN roles r ON r.id = ur.id_rol
     WHERE u.id = $1
